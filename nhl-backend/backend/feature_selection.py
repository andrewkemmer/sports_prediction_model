"""NHL moneyline RFE: record-only, deterministic, and explicitly adopted.

Structural mirror of the NFL feature_selection.py. The default production
path is unchanged. RFE evaluates the existing NHL moneyline ensemble on the
existing folds; it never changes the active feature contract unless
``--adopt`` is invoked explicitly.

Commit gate (MLB parity, 2026-09-23): RFE_NOISE_SIGMA = 1.0 — a trial
commits when its pooled logloss gain beats max(0.0005, 1.0 x paired
per-game SE). The paired construction (same games scored twice) cancels
shared per-game difficulty, so junk features self-calibrate a high bar.
"""
...
STATE_FILE = DELIVERY / "nhl_feature_selection_state.json"
TRACE_PREFIX = "nhl_feature_selection_"
WORKBOOK_PREFIX = "nhl_feature_workbook_"
...
def _score(games: pd.DataFrame, folds: list) -> tuple[dict[str, Any], np.ndarray]:
    ...
    oof = result["oof"]
    if oof.empty:
        raise RuntimeError("NHL RFE produced no OOF rows")
    ...
    if len(y) == 0:
        raise RuntimeError("NHL RFE trial produced no finite OOF predictions")
...
def run_rfe(games: pd.DataFrame, day: str, max_steps: int | None = None) -> dict[str, Any]:
    ...
    adds, unresolved_add = _resolve(_list_env("NHL_RFE_ADDITION_MONEYLINE_LIST"), candidates)
    removes, unresolved_remove = _resolve(_list_env("NHL_RFE_REMOVAL_MONEYLINE_LIST"), base)
    ...
    if not targeted:
        ...
        if not candidates:
            logger.info("NHL RFE: no declared candidates (config.RFE_CANDIDATE_COLS is "
                        "empty) — the sweep is removals-only over the %d-feature contract",
                        len(base))
    if _truthy("NHL_RFE_RETRIAL"):
    ...
    try:
        ...
        max_states = 16
    if _truthy("NHL_RFE_ADDITION_REMOVAL_GRID_MODE") and targeted:
    ...
    budget = max_steps
    if targeted:
        logger.info("NHL RFE: targeted run, budget=%d trials=%d", budget, len(order))
    else:
        logger.info("NHL RFE: full sweep, budget=%d, trial space = %d removals "
    ...
    for item in order:
        ...
        try:
            ...
            threshold = max(0.0005,
                            getattr(config, "RFE_NOISE_SIGMA", 1.0) * se)
            ...
            logger.info("NHL RFE trial %d (%s %s, n=%d): committed=%s gain=%+.5f "
                        "threshold=%.5f", len(steps), kind, item["feature"],
                        len(trial), committed, gain, threshold)
        except Exception as exc:  # noqa: BLE001
            # A trial that cannot be scored is RECORDED, never fatal.
            steps.append({"step": len(steps) + 1, "kind": kind, "feature": item["feature"],
                          "adds": item.get("adds", []), "removes": item.get("removes", []),
                          ...
                          "error": str(exc)})
            logger.warning("NHL RFE trial %d (%s %s) failed; recorded, not fatal: %s",
                           len(steps), kind, item["feature"], exc)
        finally:
            config.set_feature_subset(active)
        budget -= 1
    config.reset_feature_subset()
    record = {"schema": "nhl-rfe-v2", "date": day, "created_utc": datetime.utcnow().isoformat() + "Z",
              ...
              "commit_se_multiple": getattr(config, "RFE_NOISE_SIGMA", 1.0),
              "bar_basis": f"paired_diff_{getattr(config, 'RFE_NOISE_SIGMA', 1.0):g}sigma",
...
def maybe_run_rfe(games: pd.DataFrame, day: str) -> dict[str, Any]:
    if not _truthy("NHL_RFE_FORCE"):
        return {"ran": False, "reason": "NHL_RFE_FORCE not set"}
...
def adopt(trace_path: str | None = None) -> dict[str, Any]:
    ...
    STATE_FILE.write_text(json.dumps({"schema": "nhl-rfe-state-v1", "source_trace": path.name,
