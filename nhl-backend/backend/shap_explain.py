"""Per-game SHAP attributions for the NHL moneyline ensemble (display only).

Structural mirror of the NFL explainability contract
(``shap_game_<game_id>.csv`` → feature / shap_value / signed_effect /
perspective_team) with the NHL ensemble's semantics: attributions are
averaged across the TREE members (xgboost / lightgbm) in log-odds space and
blended with the deployed adaptive weights — the same members that own the
tree-family view of the production blend. The linear member (elastic-net) is
excluded exactly like MLB (its scaled input space has no leaf-path
attribution).
...
Output: data_delivery/nhl_shap_game_<game_id>.csv (one file per slate game).
"""
...
def compute_nhl_shap_per_game(bundle: dict, games: pd.DataFrame,
                              out_dir: Path) -> int:
    """Write one SHAP CSV per slate game from the deployed ensemble bundle.

    ``bundle`` is the persisted ``nhl_ensemble_latest.joblib`` dict (the
    ...
    try:
        ...
        logger.warning("shap not available — no NHL SHAP files written")
        return 0
    ...
    for idx, row in games.iterrows():
        ...
        # the members that produced values).
    ...
    logger.info("NHL SHAP attributions written for %d games", written)
    return written
