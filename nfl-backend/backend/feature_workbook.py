...
def _sheet_coverage(wb: Workbook, trace: dict) -> None:
    ...
    ws.append(["Feature", "Type", "Side", "Non-null games", "Coverage %",
               "Scored rows"])
    rows = []
    for name, c in cov.items():
        rows.append((float(c.get("pct", 0.0)), name, c))
    rows.sort(key=lambda t: t[0])
    for pct, name, c in rows:
        m = meta.get(name, {})
        ws.append([name, m.get("type", ""), m.get("side", ""),
                   c.get("n", 0), pct, n_rows])
    _style_table(ws, 6, [30, 22, 20, 15, 12, 12])
