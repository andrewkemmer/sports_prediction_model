"""NHL run-engine market diagnostics — the MLB ``market_diagnostics`` mirror
over the NHL decided OOF store (``kind == "oof"`` rows of
``nhl_run_engine_markets_YYYYMMDD.csv``).

The NHL artifact carries per game: the mu pair, fair spread/total (integer
medians of the margin/total PMFs from the NB Monte Carlo), FULL grids
(``p_home_cover_<±L>`` / ``p_push_<±L>`` over −8…+8 plus the half-stop
pair ±0.5; ``p_over_<U>`` / ``p_under_<U>`` over 4…12 with pushes in their
OWN ``p_push_total_<U>`` namespace), the derived-ML pair, and ACTUALS
(home/away score, total, margin) + honest outcomes (``y_over_fair`` /
``y_cover_fair`` / ``y_home_win`` …).

This module mirrors MLB's diagnostics API — same dict schemas, same table
conventions, same captions — on the NHL grids. The chart builders
...
NHL-specific semantics (documented deltas, not approximations):
  ...
  * THE OVERLAP RULE: the spread grid (−8…+8) and the totals grid (4…12)
    share the integer lines 4…8, so totals pushes are read EXCLUSIVELY from
    ``p_push_total_<U>`` (the shared ``p_push_<U>`` column is the MARGIN
    push — reading it as a totals push was the Brier-315 artifact bug the
    namespace split fixed). Every totals leg in this module (distribution
    PMF, relativized/fixed pairs, calibration cards, winner cards) reads
    ``p_push_total``; every spread leg reads ``p_push``.
...
# Integer-support grids (mirror of config.SPREAD_GRID / config.TOTAL_GRID —
# the artifact's own column families; NHL-sized, NOT the NFL ranges).
TOTAL_GRID = list(range(4, 13))           # 4 … 12 (goals totals)
SPREAD_GRID = list(range(-8, 9))          # −8 … +8 (margin thresholds)
...
# -8.0 (16 lines — the grid's full magnitude span at the NHL's ~2-goal
# margin σ; the −0.5 stop prices from the P(margin > 0.5) =
# win-probability identity, its 2-way curve IS the derived-ML curve).
SPREAD_LINE_CHOICES = [round(-0.5 - 0.5 * i, 1) for i in range(16)]
DEFAULT_RUN_MAG = -1.5   # default selector line (favorite −1.5; the
                         # canonical ±1/±2 spread band the monitor scores)
DEFAULT_TOTAL = 6
# Deep-line honesty threshold: beyond −6 the sealed evidence thins quickly
# at the NHL grid's edges — the page adds an honesty caption.
DEEP_LINE_CAPTION_MAG = 6.0

# Relativized offsets (integers — NHL totals are integer-support; ±2 keeps
# the shifted lines inside the 4…12 grid for virtually every fair total).
OFFSET_EDGES = [-2, -1, 0, 1, 2]

# Pooled fixed lines for the "Pooled lines" tab — the canonical totals the
# monitor's per-line metrics score (config.RUN_ENGINE_FIXED_TOTALS).
FIXED_TOTAL_LINES = (5, 6, 7)
...
def _col(base: str, x: float) -> str:
    """Artifact grid column tag — MLB-style (mirror of the backend's
    ``_grid_key``): '-' -> 'm', '.' -> '_', and an integral line drops its
    trailing '.0' (integer grids). ``p_home_cover_m3`` = P(margin > -3);
    ``p_home_cover_0_5`` = P(margin > 0.5); ``p_push_total_6`` =
    P(total == 6)."""
    s = str(float(x))
    if s.endswith(".0"):
        s = s[:-2]
...
def total_distribution(decided: pd.DataFrame, kmax: int = 15) -> dict[str, Any]:
    ...
    artifact's per-game ``p_push_total_<U>`` columns (P(total == U); the
    totals-push namespace, never the margin's ``p_push_<U>``); observed =
    ...
    for U in TOTAL_GRID:
        col = _col("p_push_total", U)
        if col not in decided.columns:
            continue
        if U <= kmax:
            modeled[U] = float(decided[col].mean())
    # normalized (grid mass may not capture < 4 / > 12 exactly; keep both
    ...
    return {
        ...
        "callouts": {
            "P(total<=4)": {"observed": round(float(observed[:5].sum()), 4),
                            "modeled": round(float(modeled[:5].sum()), 4)},
            "P(total>=10)": {"observed": round(float(observed[10:].sum()), 4),
                             "modeled": round(float(modeled[10:].sum()), 4)},
            "note": ("Per-game total PMFs from the NB Monte Carlo score "
                     "draws; integer-support means the whole-number mass "
                     "IS the push band."),
        },
...
def relativized_pairs(decided: pd.DataFrame,
    ...
    pooled probability axis spans the range. NHL integer lines: pushes
...
def game_total_calibration(decided: pd.DataFrame,
    ...
    game_total_calibration with the NHL integer-support push band.
    """
...
def _favorite_cover(decided: pd.DataFrame, mag: float
    ...
    mag may be a half-integer (0.5 … 7.5): the -0.5 stop prices from the
    ...
    (push = P(margin == ±m), read from the margin-push namespace)."""
...
# mirrors over the NHL decided store)
# ---------------------------------------------------------------------------
...
def _card_metrics(y: np.ndarray, p: np.ndarray) -> dict[str, Any]:
    ...
    ll = _logloss(y, p)
    out["logloss"] = round(ll, 4) if ll is not None else None
...
def winner_cards(decided: pd.DataFrame) -> dict[str, Any]:
    ...
    from the decided OOF store: pooled = all rows, holdout = sealed
    ...
    def _pairs(kind: str, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, r in df.iterrows():
            if kind == "over_under":
                ...
                # under leg is 1 - over - push (push read from the
                # TOTALS-push namespace p_push_total_<U>; the shared
                # p_push_<U> column is the MARGIN push and is never read
                # as a totals leg). 2-way no-push re-scale.
                ...
                if not np.isfinite(po):
                    continue
                pp_col = _col("p_push_total", u_int)
...
def totals_monitor_stats(decided: pd.DataFrame, min_pct: float = 0.0,
    ...
    for _, r in decided.iterrows():
        ...
        # approximation); under = 1 - over - push from the TOTALS-push
        # namespace (p_push_total_<U>).
        po = _fnum(r, "p_over_fair")
        U = _fnum(r, "fair_total")
        tot = _fnum(r, "total")
        if not np.isfinite(U) or not np.isfinite(tot):
...
def load_run_engine_csv(ds: str, prefix: str) -> pd.DataFrame | None:
    ...
    try:
        raw, _src = utils._fetch_bytes(fname, sport="nhl", **cfg)
    except Exception:
        raw = None
    if raw is None:
        return None
...
def render_run_engine_drift(drift: pd.DataFrame | None) -> None:
    ...
    for r, w in zip(records, weight_pcts):
        ...
        label = utils.describe_feature(r.get("feature", ""), sport="nhl") \
            or r.get("feature", "")
...
def fold_slate_history(monitors: list[dict]) -> dict[str, list[dict]]:
    ...
    Each dated ``nhl_run_engine_monitor_<date>.json`` ships its own
...
def runline_monitor_stats(decided: pd.DataFrame,
    ...
    semantics (margin == ±m pushes excluded; the push read from the
    margin-push namespace)."""
    out = {"n": 0, "n_wins": 0, "n_losses": 0, "n_pushes": 0,
