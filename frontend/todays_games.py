...
# away-favorite columns when they
# exist, so calibrated production pricing is shown verbatim. Legacy snapshots
# without those columns retain the raw NB fallback and are marked internally
# as legacy-derived until regenerated by the canonical pipeline.
...
def _orient_rl_bits(bits, row, fav_home: bool) -> dict:
    ...
    home); fav_home=False first consumes the persisted calibrated
    AWAY -L / HOME +L columns when present, then falls back to the same
    distributional calculation for legacy artifacts that predate that
    contract. Returns a shallow copy; the caller's bits are not mutated.
"""
    out = dict(bits or {})
    ...
    if bool(fav_home):
        ...
        return out
    if bits.get("rl_unverified") or row is None or getattr(row, "empty", False):
        return out
    line = bits.get("rl_line") or 1.5
    # Prefer the backend's persisted, calibrated opposite-tail contract.
    # Older snapshots may not carry these columns; only then use the
    # distributional fallback below. This keeps the card identical to the
    # production artifact whenever canonical pricing is available.
    key = f"{float(line):.1f}".replace(".", "_")
    fav_col = f"p_rl_{key}_away_favorite"
    push_col = f"p_rl_{key}_away_push"
    dog_col = f"p_rl_{key}_home_dog"
    fav_cov = diag._num(row, fav_col)
    dog_cov = diag._num(row, dog_col)
    ...
    if fav_cov is not None and dog_cov is not None:
        ...
        return out
    # Legacy artifacts lack the opposite-tail columns. Preserve the existing
    # PIT-safe fallback for those snapshots, but mark its provenance so it is
    # never mistaken for persisted calibrated artifact pricing.
    ...
    out["rl_fav_source"] = "legacy_raw_derived"
    return out
