...
(lineup_actual_woba_delta / lineup_actual_top3_delta / lineup_rest_count, per side)
help the ensemble out of sample? Nothing is changed silently; the feature set
ships only if it clears the sealed-holdout gate.
...
- Data: committed data_delivery/game_level_features.csv (4,451 games, hash
  ...
  real on this artifact (unlike the momentum deltas, which were 4/38 real).
- Variants: WITHOUT = the original 58 FEATURE_COLS (production baseline);
  WITH = 58 + the 6 lineup-delta columns.
...
Emits data_delivery/lineup_ablation_<sha>.json (incremental — resumes by
skipping variants already present). COMMITS NOTHING.

Usage:
    python run_lineup_ablation.py
    python run_lineup_ablation.py --variants WITHOUT,WITH
"""
...
def build_variants() -> dict[str, list[str]]:
    """WITHOUT = original 58; WITH = 58 + the 6 lineup-delta columns."""
...
def main() -> None:
    ...
    if out.exists():
        ...
        results = {"schema": "lineup-ablation/v1", "commit_sha": sha,
